...
public class HSQLLifeCycle implements WebApplicationLifecycle {
    ...
    private void createTables(){
        ...
        try {
            statement = hibernateDaoHelper.getConnection().createStatement();
            // create group
            statement.executeUpdate("create table t_group (id bigint not null primary key,name varchar,createdby_id bigint, created_date date,updatedby_id bigint,updated_date date)");
            // create contact
            statement.executeUpdate("create table t_contact (id bigint not null primary key,name varchar,address varchar,phone varchar,email varchar,createdby_id bigint, created_date date,updatedby_id bigint,updated_date date)");
            // create group contact
            statement.executeUpdate("create table t_group_contact (id bigint not null primary key,group_id bigint, contact_id bigint)");
        } catch (SQLException e) {
            e.printStackTrace();
        } finally{
            if(statement != null){
